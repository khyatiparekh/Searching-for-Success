...
prediction_model and prediction_stock_price. The first function, featues_selection will
use Lasso regression model to select 6 keywords that are significant
...
after upcomming earning report released for given company. And the third function prediction _stock_price 
will call the previous functions based on the company name input.
"""
...
def prediction_model(X, y):
    ...
    ----------
    X: Dataframe.
    Data from Google Trends with keywords selected from feature_selection function only.
...
def prediction_stock_price (company):
    """Given Google trends data on several keywords and the stock price performance on 
    same company, this will output the likelihood of the stock price increase for 
    input company.
    ...
    X: Dataframe.
    Data downloaded from Google Trends of standardized daily searched 
    counts for selected keywords.Independent Variables. GoogleTrends.csv, MicrosoftTrends.csv and
    AmazonTrends.csv will be used based on the company selected.
    y: Array.
    Standardized daily stock prices for given company.
    ...
    ------
    Output: Statement
    The likelihood of stock price increase for input company. Value between 0 and 1.
