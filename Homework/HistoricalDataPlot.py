...
def historical_data_plot(company):
    ...
    output: Interactive Plot of standardized Google daily searched counts of input keywords.
            And interactive Plot of daily closed stock price for selected company.
    """
    ...
    q = TimeSeries(data['Price'],title="Daily Stock Adjusted Close Price", ylabel='Stock Prices', xlabel='Date')
    show(p)
    show(q)